<script lang="ts">
    ...
    async function btnSearchClicked() {
        ...
        for await (const idTextIRI of selectedItems) {
            ...
            //console.log(capa)
            let objIdTextIRICapability = {id: idTextIRI.id, text: idTextIRI.text, iri: idTextIRI, capabilities: capa }
...
<form class="m-2">
    <div class="flex items-center mb-1 text-sm font-medium text-gray-900 dark:text-gray-400">
        <label for="instituicoes_multiple" class="mr-2">Escolha as instituições</label>
        <input class="mr-2" type="checkbox" bind:checked={isSelectedAll}> 
        <p class="mr-2">selecione todos</p>
        <button class="mr-2 focus:outline-none bg-grey-light hover:bg-grey text-grey-darkest font-bold rounded inline-flex items-center hover:bg-gray-100" on:click|preventDefault={btnSearchClicked} title="Buscar camadas">
        ...
        <p class="ml-auto text-sm ">Quantidade de camadas: {qtdToTalCamadas}</p>
    </div>
    <select multiple id="instituicoes_multiple" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" bind:value={selectedItems}>
...
<div class = "mt-2 grid gap-2 md:grid-cols-3 grid-cols-1">
     
        {#each objIdTextIRICapabilitiesArray as wmsCapa}
            <WMSCapabilitiesCard wmsCapabilities={wmsCapa.capabilities} capabilitiesUrl= {wmsCapa.iri} capabilitiesText={wmsCapa.text}></WMSCapabilitiesCard>
        {/each}    
